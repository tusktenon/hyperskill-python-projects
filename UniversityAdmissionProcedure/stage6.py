...
def read_applicants(datafile):
    ...
    with open(datafile) as file:
        for line in file:
            ...
            grades = GradeSet(float(chem), float(comp_sci), float(math), float(physics))
    ...
    for priority in range(3):
        for department in accepted:
            sorted_applicants = sorted(
                list(applicants),
                key=lambda a: (-a.grades.for_dept(department), a.name),
            )
            for applicant in sorted_applicants:
                ...
                if preference == department and len(accepted[preference]) < capacity:
                    accepted[preference].append(
                        (applicant.name, applicant.grades.for_dept(department))
                    )
...
def main():
    capacity = int(input())
    applicants = read_applicants('data/stage5/example.txt')
